...
const (
	...
	instructionButton  = "2. Покрокова інструкція"
...
Далі зазначаємо: "Відповідь прошу надіслати на електронну адресу _________ або на поштову адресу _________." (можете вказати поруч зі своєю ел.пошту так і ел.пошту КП НМР "ЖКО", аби відповідь прийшла безпосередньо до нас)`
...
Сфотографуйте заяву та разом із ксерокопіями паспорта надішліть на електронну адресу Державної прикордонної служби України: adpsu@dpsu.gov.ua`
	instructionStepSixText = `Шостий крок 👇🏻
Готово! Тепер мешканцю квартири, у якій Ви зареєстровані, необхідно відвідати КП НМР "ЖКО" для подання додаткової заяви з метою проведення перерахунку безпосередньо для квартири. 
Заяву принести за адресою: Незалежності 31, адмінприміщення КП НМР "ЖКО" (вхід навпроти церкви): 
📍пн - чт - з 8.00 до 17.15 (обідня перерва 12.00-13.00)
📍пт  - з 8.00 до 16.00 (обідня перерва 12.00-13.00)
Або надіслати на e-mail: lawyerjko@gmil.com
Або на вайбер: 097 900 67 00
❗️Важливо: заява повинна бути підписана заявником особисто
...
func startPeriodicRequest(url string) {
	...
	go func() {
		for range ticker.C {
			...
			resp.Body.Close()
			log.Printf("Periodic request to %s completed", url)
		}
	}()
}
...
func main() {
	...
	startPeriodicRequest("https://telegram-bot-976280914316.us-central1.run.app") // Replace with your URL
